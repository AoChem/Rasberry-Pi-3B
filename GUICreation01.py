...
def model_training_main():
	...
	para_init_button = Button(mt, text='Initializing Parameters...', command=lambda: model_training_init(mt)).pack()
	select_button = Button(mt, text='Select Dataset...', command=lambda: dataset_selecting(mt)).pack()
	train_button = Button(mt, text='Train', command=lambda: model_training(display_name, model_name, model_filename, csv_name)).pack()
